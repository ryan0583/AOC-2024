...
const parseMultiplicationInstruction = (instruction: string) => {
    ...
    const limitedLimitedInstruction = limitedInstruction.slice(0, lastBracketIndex + 1);
    const multiplicationNumbers = limitedLimitedInstruction.replaceAll('(', '').replaceAll(')', '').split(',').map(Number);
    return (multiplicationNumbers[0] * multiplicationNumbers[1]) || 0;
}

const multiplied = multiplicationInstructions.map(parseMultiplicationInstruction)
...
const multiplied2 = multiplicationInstructions.map((instruction) => {
    const multiplicationResult = enabled ? parseMultiplicationInstruction(instruction) : 0;
